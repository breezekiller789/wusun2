...
void HashInit(struct kvIdb *kp, int maxHash, int keyBufSize){
    struct hnode **HashTab;
    kp->HashTab = (struct hnode **)malloc(sizeof(struct hnode *) * maxHash);
    if(kp->HashTab == NULL){
    ...
    kp->keyBuf = (uchar *)malloc(keyBufSize);
    kp->kptr = kp->keyBuf;
}
...
/*         , char **kptr){ */
void HashInsert(struct kvIdb *kp, char *key){
        
    int32 hashv;   // hash value
    char *ptr, *qtr; 
    struct hnode *np, *tmp;
    hashv = hash33(key) % kp->maxHash;
    np = kp->HashTab[hashv];
